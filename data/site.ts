export const siteConfig = {
  name: "Automate4U",
  url: "https://automate4u.co",
  defaultTitle: "Automate4U - AI Voice, Agents, and Workflow Automation",
  description:
    "Automate4U builds AI voice agents, chat and email agents, and workflow automations that connect to the systems running your business.",
  primaryCta: {
    label: "Get Free Assessment",
    offerName: "Free AI Workflow Assessment",
    href: "/contact",
  },
};

export const staticRoutes = [
  "/",
  "/contact",
  "/pricing",
  "/privacy",
  "/terms",
  "/services",
  "/services/ai-voice",
  "/services/ai-agents",
  "/services/ai-chat",
  "/services/ai-transformation",
  "/services/marketing-automation",
  "/services/managed-ai-operations",
  "/services/custom-ai-software",
  "/capabilities",
  "/capabilities/custom-software-ai-solutions",
  "/capabilities/data-integrations-infrastructure",
  "/capabilities/operational-intelligence-analytics",
  "/capabilities/security-monitoring-ai-safety",
  "/capabilities/technology-strategy-ai-enablement",
  "/capabilities/training-prompting-ai-mastery",
  "/solutions",
  "/solutions/daycare-voice-agent",
  "/solutions/manufacturing-quote-order-agent",
  "/solutions/healthcare-front-desk-agent",
  "/solutions/home-services-missed-call-dispatch-agent",
  "/solutions/law-firm-intake-agent",
  "/solutions/accounting-firm-intake-agent",
  "/solutions/wealth-management-intake-agent",
  "/solutions/hvac-service-call-agent",
  "/solutions/plumbing-service-call-agent",
  "/solutions/electrical-service-call-agent",
  "/solutions/chiropractic-pt-front-desk-agent",
  "/solutions/med-spa-front-desk-agent",
  "/solutions/mental-health-counseling-intake-agent",
  "/solutions/montessori-preschool-voice-agent",
  "/solutions/tutoring-learning-center-agent",
  "/solutions/wholesale-distribution-order-agent",
  "/solutions/industrial-supplier-order-agent",
  "/industries",
  "/industries/education-childcare",
  "/industries/healthcare",
  "/industries/home-services",
  "/industries/hospitality",
  "/industries/manufacturing",
  "/industries/professional-services",
  "/about/overview",
  "/about/our-mission",
  "/about/our-team",
  "/about/how-we-work",
];
